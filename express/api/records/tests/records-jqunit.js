...
var namespace = "gpii.ctr.record.tests.records";
...
records.startExpress = function() {
    ...
    records.express.start(function() {
        ...
        records.express.app.use('/records', require('../../records')(records.config));

        var termsConfig = JSON.parse(JSON.stringify(records.config));
        termsConfig.recordType = "term";
        var terms = require("../../records")(termsConfig);
        records.express.app.use('/terms', terms);

        var aliasConfig = JSON.parse(JSON.stringify(records.config));
        aliasConfig.recordType = "alias";
        var aliases = require("../../records")(aliasConfig);
        records.express.app.use('/aliases', aliases);

        var transformConfig = JSON.parse(JSON.stringify(records.config));
        transformConfig.recordType = "transform";
        var transforms = require("../../records")(transformConfig);
        records.express.app.use('/transforms', transforms);

        var translationConfig = JSON.parse(JSON.stringify(records.config));
        translationConfig.recordType = "translation";
        var translations = require("../../records")(translationConfig);
        records.express.app.use('/translations', translations);

        var operatorsConfig = JSON.parse(JSON.stringify(records.config));
        operatorsConfig.recordType = "condition";
        var operators = require("../../records")(operatorsConfig);
        records.express.app.use('/conditions', operators);

        records.runTests();
    });
};

records.runTests = function() {
    ...
    allEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Load endpoint '" + endPoint + "' with no arguments", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint, function(error, response, body) {
                    jqUnit.start();
...
// Make sure that "total_rows" is meaningful
    allEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Sanity check total_rows for endpoint '" + endPoint + "'...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?limit=-1", function(error, response, body) {
                    jqUnit.start();
...
// Test paging by asking for records 1-2 and then records 2-2 and comparing record 2 to itself
    recordTypeEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Testing paging w/ endpoint '" + endPoint + "' ...", function() {
                ...
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?offset=0&limit=2", function(error, response, body) {
                    jqUnit.start();
                    ...
                    request.get("http://localhost:" + records.config.port + "/" + endPoint + "?offset=1&limit=1", function(error, response, body) {
                        jqUnit.start();
...
// Test sorting (all record types)
    recordTypeEndPoints.forEach(function(endPoint){
        jqUnit.asyncTest("Testing sorting for " + endPoint + " records...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?&sort=termLabel",function(error, response, body) {
                    ...
                    request.get("http://localhost:" + records.config.port + "/" + endPoint + "?sort=%5CtermLabel",function(error, response, body) {
                        jqUnit.start();
                        testUtils.isSaneResponse(jqUnit, error, response, body);
...
// Test sorting (terms, with children)
    jqUnit.asyncTest("Testing term sorting, with children", function() {
            request.get("http://localhost:" + records.config.port + "/terms?sort=termLabel",function(error, response, body) {
                ...
                request.get("http://localhost:" + records.config.port + "/terms?sort=%5CtermLabel",function(error, response, body) {
                    jqUnit.start();
                    testUtils.isSaneResponse(jqUnit, error, response, body);
...
// Limit by one status, and then limit by two.  There should be more active and deleted records than just active records
    recordTypeEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Testing filtering by status w/ endpoint '" + endPoint + "' ...", function() {
                ...
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?status=active", function(error, response, body) {
                    jqUnit.start();
                    ...
                    request.get("http://localhost:" + records.config.port + "/" + endPoint + "?status=active&status=unreviewed&status=deleted", function(error, response, body) {
                        jqUnit.start();
...
// There should be records updated since the year 2000
    recordTypeEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Filter endpoint '" + endPoint + "' by future date...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?updated=2000-01-01", function(error, response, body) {
                    jqUnit.start();
...
// Confirm that /api/records?recordType=term returns the same number of records as /api/terms
    recordTypeEndPoints.forEach(function(endPoint) {
        ...
        jqUnit.asyncTest("Testing endpoint '" + endPoint + "' versus records?recordType=" + recordType + "...", function() {
            ...
            request.get("http://localhost:" + records.config.port + "/records?recordType=" + recordType, function(error, response, body) {
                jqUnit.start();
...
// Confirm that the same number of terms are returned with and without the "children" option
    jqUnit.asyncTest("Testing terms with and without children...", function() {
        var firstRecordCount, secondRecordCount, firstTotalRows, secondTotalRows;
        request.get("http://localhost:" + records.config.port + "/terms", function(error, response, body) {
            jqUnit.start();
            ...
            request.get("http://localhost:" + records.config.port + "/terms?children=true" , function(error, response, body) {
                jqUnit.start();
...
// There should be no records updated in the year 3000
    recordTypeEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Filter endpoint '" + endPoint + "' by future date...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?updated=3000-01-01", function(error, response, body) {
                    jqUnit.start();
...
// When using the children option, "updated" should also be respected
    jqUnit.asyncTest("Terms with children, limited by future date...", function() {
        request.get("http://localhost:" + records.config.port + "/terms?children=true&updated=3000-01-01", function(error, response, body) {
            jqUnit.start();
...
// When using the children option, the "status" field should work correctly
    records.config.allowedStatuses.forEach(function(status){
        jqUnit.asyncTest("Terms w/ children, limited by status '" + status + "'...", function() {
            request.get("http://localhost:" + records.config.port + "/terms?children=true&status=" + status, function(error, response, body) {
                jqUnit.start();
...
// Test that modules other than "records" do not support the recordType option
    recordTypeEndPoints.forEach(function(endPoint){
            jqUnit.asyncTest("Query endpoint '" + endPoint + " with no arguments...", function() {
                jqUnit.start();
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?recordType=general", function(error, response, body) {
...
// Test multi-sort functionality
    recordTypeEndPoints.forEach(function(endPoint) {
        jqUnit.asyncTest("Testing multi-sort of '" + endPoint + "'...", function() {
            ...
            request.get("http://localhost:" + records.config.port + "/" + endPoint + "?sort=status&sort=uniqueId", function(error, response, body) {
                jqUnit.start();
...
// Test multi-sort functionality with ?children=true
    jqUnit.asyncTest("Testing multi-sort of terms with children=true...", function() {
        var firstRecordCount, secondRecordCount, firstTotalRows, secondTotalRows, firstData, secondData;
        request.get("http://localhost:" + records.config.port + "/records?recordType=term&sort=status&sort=uniqueId", function(error, response, body) {
            jqUnit.start();
...
// The output from "terms" and "records" should not include children with the "children" option set to false
    ["records","terms"].forEach(function(endPoint){
            jqUnit.asyncTest("Query endpoint '" + endPoint + " with the 'children' argument set to false...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?children=false", function(error, response, body) {
                    jqUnit.start();
...
// The output from "terms" and "records" should include children when the "children" option is set to true
    ["records","terms"].forEach(function(endPoint){
            jqUnit.asyncTest("Query endpoint '" + endPoint + " with the 'children' argument set to true...", function() {
                request.get("http://localhost:" + records.config.port + "/" + endPoint + "?children=true", function(error, response, body) {
                    jqUnit.start();
